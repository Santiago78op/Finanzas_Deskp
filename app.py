...
def _ahorro_con_saldo(conn, fila, promedio=None, hoy=None):
...
@app.post("/api/ahorros/aplicar-plan")
def aplicar_plan():
    """
    ...
    try:
        ...
        for a in plan["asignaciones"]:
            conn.execute(
                ...
                (hoy, a["ahorro_id"], a["mensual"], "Plan sugerido"),
            )
        conn.commit()
...
@app.delete("/api/ahorros/{ahorro_id}")
...
@app.get("/api/ahorros/{ahorro_id}/aportes")
...
@app.post("/api/ahorros/{ahorro_id}/aportes")
...
@app.delete("/api/ahorros/aportes/{aporte_id}")
